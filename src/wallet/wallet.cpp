...
bool CWallet::Unlock(const SecureString& strWalletPassphrase, bool fForPosOnly)
{
...

    {
        ...
        for (const MasterKeyMap::value_type& pMasterKey : mapMasterKeys)
        {
            ...
            if (CCryptoKeyStore::Unlock(_vMasterKey, fForPosOnly))
                return true;
        }
    }
    return false;
                                 ...
                                 std::set<CInputCoin>& setCoinsRet, CAmount& nValueRet, const CoinSelectionParams& coin_selection_params, bool& bnb_used) const
{
    ...
    if (coin_selection_params.use_bnb) {
        ...
        CFeeRate long_term_feerate = GetMinimumFeeRate(::mempool);

        // Calculate cost of change
        CAmount cost_of_change = dustRelayFee.GetFee(coin_selection_params.change_spend_size) + coin_selection_params.effective_fee.GetFee(coin_selection_params.change_output_size);
                         ...
                         int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    ...
    int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            AvailableCoins(vAvailableCoins, true, &coin_control, 0);
            ...
            CFeeRate nFeeRateNeeded = GetMinimumFeeRate(coin_control, ::mempool);
            ...
            while (true)
            {
                ...
                nFeeNeeded = GetMinimumFee(nBytes, ::mempool);
                ...
                if (nFeeRet >= nFeeNeeded) {
                    ...
                    if (nChangePosInOut == -1 && nSubtractFeeFromAmount == 0 && pick_new_inputs) {
                        ...
                        CAmount fee_needed_with_change = GetMinimumFee(tx_size_with_change, ::mempool);
...
bool CWallet::CreateStakeTransaction (CBlockHeader& header, CMutableTransaction &txNew, CAmount& nPosReward)
{
    ...
    if ((stakeCoins.size() == 0) || (nCoinStakeTime - stakeLastUpdateTime > 120)) {
        stakeCoins.clear();
        AvailableCoins(stakeCoins, true, nullptr, 0);
        LogPrint(BCLog::SELECTCOINS, "        Total update select coin = %d\n", stakeCoins.size());
        stakeLastUpdateTime = nCoinStakeTime;
    }

    int32_t nSearchInterval = nCoinStakeTime - stakeLastSearchTime;
    if (nSearchInterval > 30) nSearchInterval = 30;
    stakeLastSearchTime = nCoinStakeTime;
    ...
    for (const COutput& inpcoin : stakeCoins) {
        ...
        for (uint32_t n = 0; n < nSearchInterval && !fKernelFound; n++) {
            ...
            if (CheckProofOfStake (header, consensus, pcoin.outpoint, pcoin.txout.nValue, time)) {
                ...
                if (isStakeRepeatAddr) {
                    ...
                    CPubKey vchPubKey;
                    SetUsedForPoS (true);
                    bool ret = key0.GetReservedKey(vchPubKey, true);
                    SetUsedForPoS (false);
                    if (!ret) return error("CreateCoinStake: Keypool ran out, please call keypoolrefill first");
                    LearnRelatedScripts(vchPubKey, DEFAULT_ADDRESS_TYPE);
    ...
    while(true) {
        ...
        SetUsedForPoS (true);
        bool ret = SignTransaction(txNew);
        SetUsedForPoS (false);
        if (!ret)
            return error("CreateCoinStake : failed to sign coinstake");

        // Limit size
        size_t nBytes = ::GetSerializeSize(txNew, SER_NETWORK, PROTOCOL_VERSION);
        if (nBytes >= MAX_BLOCK_WEIGHT/10)
        ...
        if (nMinFee < nFee) {
            ...
            LogPrint(BCLog::SELECTCOINS, "CreateCoinStake : fee for coinstake %s\n", FormatMoney(nMinFee));
            break;
        }
    }
    nPosReward += nMinFee; // recover paid fee in coinbase transaction
    return true;
}
...
bool CWallet::SignBlock (CBlock& block) {
    ...
    if (whichType == TX_PUBKEY) {
        ...
        SetUsedForPoS (true);
        bool ret = GetKey(pubKey.GetID(), key);
        SetUsedForPoS (false);
        if (!ret) 
            return false;
...
bool CWallet::ReserveKeyFromKeyPool(int64_t& nIndex, CKeyPool& keypool, bool fRequestedInternal)
{
    ...
    keypool.vchPubKey = CPubKey();
    {
        ...
        if (IsLocked(fUsedPoS)) return false;
        TopUpKeyPool();
...
void CWallet::postInitProcess()
{
    ...
    const CAmount& mn_coins = Params().GetConsensus().nMasternodeAmountLock * COIN;
    std::vector<COutput> vCoins;
    AvailableCoins(vCoins, true, nullptr, mn_coins, mn_coins);
    for (const COutput& out : vCoins)
        LockCoin (out.GetInputCoin().outpoint);
}
