...
void InitParameterInteraction()
{
    ...
    if (gArgs.GetBoolArg("-masternode", false)) {
        ...
        if (gArgs.SoftSetBoolArg("-listen", true))
            LogPrintf("%s: parameter interaction: -masternode set -> setting -listen=1\n", __func__);
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if wallet is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if wallet is started while in a temporary directory.\n",
    ...
    if (chainparams.GetConsensus().WitnessHeight > 0) {
