...
#if defined(NDEBUG)
# error "Vqr cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Vqrc Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s Vqr additional fees, %d delta bytes\n",
...
bool GetTransaction(const uint256& hash, CTransactionRef& txOut, const Consensus::Params& consensusParams, uint256& hashBlock, bool fAllowSlow, CBlockIndex* blockIndex)
{
    ...
    if (!blockIndex) {
        ...
        if (fTxIndex) {
            ...
            try {
                file >> header;
                if (fseek(file.Get(), postx.nTxOffset, SEEK_CUR)) return error("%s: fseek(...) failed", __func__);
                file >> txOut;
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    ...
    // Check the header
    if (block.IsProofOfWork() && !CheckProofOfWork(block.GetBlockHeader(), consensusParams))
...
{
    if (nHeight <= consensusParams.LastPoWHeight) {
        return 500 * COIN;
    }
    return 0;
}
...
static unsigned int GetBlockScriptFlags(const CBlockIndex* pindex, const Consensus::Params& consensusparams) {
    ...
    unsigned int flags = SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_NULLDUMMY;
    ...
    if ((consensusparams.CSVHeight >= 0) && (pindex->nHeight >= consensusparams.CSVHeight)) {
        flags |= SCRIPT_VERIFY_CHECKSEQUENCEVERIFY;
    }

    if ((consensusparams.WitnessHeight >= 0) && (pindex->nHeight >= consensusparams.WitnessHeight)) {
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (block.IsProofOfStake()) {
        if (block.vtx.size() < 2) {
            return error("ConnectBlock() : Incorrent PoS block");
        }
        ...
        if (!CheckProofOfStake (block.GetBlockHeader(), consensus, prevout, coin.out.nValue, time)) {
    ...
    if (!CheckBlock(block, state, consensus, pindex->pprev, !fJustCheck, !fJustCheck)) {
        if (state.CorruptionPossible()) {
    ...
    if ((chainparams.GetConsensus().CSVHeight >= 0) && (pindex->nHeight >= chainparams.GetConsensus().CSVHeight)) {
    ...
    if (block.IsProofOfWork()) {
        ...
        blockReward += GetProofOfStakeReward(pindex->nHeight, chainparams.GetConsensus());
    }
...
bool CheckProofOfStake (const CBlockHeader& block, const Consensus::Params& params, const COutPoint &out, 
    ...
    if ((block.nTime < time) || (block.nTime - time < params.nCoinAgeTick*3)) return false;
    arith_uint256 bnTargetHi = (~arith_uint256 (0)) >> 8;
    arith_uint256 bnTargetLo = bnTarget;
    bnTarget *= std::min((int)((block.nTime - time) / params.nCoinAgeTick), 255);
    bnTarget *= std::min((int)(value / COIN), 9999);
    if (bnTarget > bnTargetHi) bnTarget = bnTargetHi;
    if (bnTarget < bnTargetLo) bnTarget = bnTargetHi;
    uint256 hash = block.GetPoSHash (out);
    LogPrint(BCLog::SELECTCOINS, "CheckProofOfStake(): TimeDiff=%d, hash=%s, target=%s\n",
        (block.nTime-time)/60, hash.ToString().substr(0,16), bnTarget.ToString().substr(0,16));
        ...
        if (fDoFullFlush || fPeriodicWrite) {
            ...
            // Then update all block file information (which may refer to block and undo files).
            {
                ...
                if (fAddressIndex && !pblockaddressindex->Flush()) {
                    return AbortNode(state, "Failed to write addresses index");
                }
                CleanAddressInfo ();
            }
...
static bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, 
    ...
    // Check proof of work matches claimed amount
    if (fCheckPOW && !block.IsProofOfStake() && !CheckProofOfWork(block, consensusParams))
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, CBlockIndex* pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (!CheckBlockHeader(block, state, consensusParams, pindexPrev, fCheckPOW))
        return false;
...
bool IsWitnessEnabled(const CBlockIndex* pindexPrev, const Consensus::Params& params)
{
    ...
    return (params.WitnessHeight >= 0) && (pindexPrev->nHeight >= params.WitnessHeight);
}
...
std::vector<unsigned char> GenerateCoinbaseCommitment(CBlock& block, const CBlockIndex* pindexPrev, const Consensus::Params& consensusParams)
{
    ...
    if (consensusParams.WitnessHeight > 0) {
        if (commitpos == -1) {
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    if (block.IsProofOfWork() && (nHeight >= consensusParams.LastPoWHeight)) 
        return state.Invalid(false, REJECT_INVALID, "consensus-pow-error", "pow now allowed"); 
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    int nLockTimeFlags = 0;
    if ((consensusParams.CSVHeight >= 0) && (pindexPrev->nHeight >= consensusParams.CSVHeight)) {
    ...
    if ((consensusParams.WitnessHeight >= 0) && (pindexPrev->nHeight >= consensusParams.WitnessHeight)) {
        int commitpos = GetWitnessCommitmentIndex(block);
        if (commitpos != -1) {
    ...
    if (!fHaveWitness) {
      for (const auto& tx : block.vtx) {
        ...
        bool exist = false;
        for (const auto& tx : block.vtx) { if (tx->HasWitness()) { exist = true; } }
        if (!exist) return state.DoS(100, false, REJECT_INVALID, "no-witness-data", true, strprintf("%s : no witness data found", __func__));
    }
...
bool ProcessNewBlock(const CChainParams& chainparams, const std::shared_ptr<const CBlock> pblock, bool fForceProcessing, bool *fNewBlock)
{
...

    {
        ...
        // belt-and-suspenders.
        bool ret = CheckBlock(*pblock, state, chainparams.GetConsensus(), pindex);
...
bool TestBlockValidity(CValidationState& state, const CChainParams& chainparams, const CBlock& block, CBlockIndex* pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    LogPrintf("TestBlockValidity(): nPowHeight = %d\n",indexDummy.nHeight);
